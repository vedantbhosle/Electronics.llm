  ...
  for (let i = 0; i < 2; i++) {
    ...
    const projectLinks = await page.evaluate(() => {
      return Array.from(document.querySelectorAll(".views-field-title a"))
        .map((a) => a.href)
        .filter((href) =>
          href.startsWith("https://circuitdigest.com/microcontroller-projects/")
        );
    });
    ...
    for (let link of projectLinks) {
      ...
      const projectData = await page.evaluate(() => {
        ...
        return {
          title: document.querySelector("h1")?.innerText.trim() || "Untitled",
          content: parts.join("\n\n"),
          codeblocks: codes,
        };
      });
      ...
      if (projectData) {
        const record = { url: link, ...projectData };
